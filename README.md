# 🎓 Курсовой проект: Поиск и первичный анализ наборов данных

<div align="center">

![Python](https://img.shields.io/badge/Python-3.9+-blue?logo=python&style=for-the-badge)
![License](https://img.shields.io/badge/License-CC%20BY--NC%204.0-lightgrey?style=for-the-badge)
![GitHub](https://img.shields.io/badge/GitHub-Repository-black?logo=github&style=for-the-badge)

**Студент:** Линьков Андрей Дмитриевич | **Группа:** ЕТ-141 | **Преподаватель:** Краева Яна Александровна

</div>

---

## 📌 О проекте

В рамках курсового проекта выполнен первичный анализ наборов данных **пяти различных типов** с применением методов машинного обучения и обработки данных.

| № | Тип данных | Задача | Датасет |
|---|------------|--------|---------|
| 1 | **📊 Табличные данные** | Обнаружение мошеннических транзакций | Credit Card Fraud Detection |
| 2 | **📈 Временные ряды** | Прогнозирование цен акций | NVIDIA Daily Stock Prices |
| 3 | **🖼️ Изображения** | Классификация защитных касок | Hard Hat Workers Dataset |
| 4 | **📝 Текстовые данные** | Анализ обращений граждан | Обращения граждан (синтетический) |
| 5 | **🎵 Аудиоданные** | *(будет добавлен в следующей версии)* | — |

---

## 📁 Структура репозитория


Course-Project/
│
├── 📁 code/                         # Исходный код на Python
│   ├── chapter1_table.py            # Анализ табличных данных
│   ├── chapter2_timeseries.py       # Анализ временных рядов
│   ├── chapter3_images.py           # Анализ изображений
│   └── chapter4_text.py             # Анализ текстовых данных
│
├── 📁 data/                         # Данные (файлы не загружены)
│   ├── table/                       # Credit Card Fraud Detection
│   ├── timeseries/                  # NVIDIA Stock Prices
│   ├── images/                      # Hard Hat Workers Dataset
│   └── text/                        # Обращения граждан
│
├── 📁 results/                      # Результаты анализа
│   ├── fig1_*.png                   # Рисунки главы 1
│   ├── fig2_*.png                   # Рисунки главы 2
│   ├── fig3_*.png                   # Рисунки главы 3
│   └── fig4_*.png                   # Рисунки главы 4
│
├── 📁 docs/                         # Документация
│   └── Отчет_курсовая_работа.docx   # Полный отчет
│
├── .gitignore                       # Игнорируемые файлы
└── README.md                        # Описание проекта


---

## 🚀 Как запустить код

### 1️⃣ Установка Python

| Шаг | Действие |
|-----|----------|
| 1 | Перейдите на [официальный сайт Python](https://www.python.org/downloads/) |
| 2 | Скачайте установщик для Windows |
| 3 | Запустите установку, **обязательно** отметьте "Add Python to PATH" |

---

### 2️⃣ Установка библиотек

Откройте **командную строку** (Win+R → cmd) и выполните команду:

```bash
pip install pandas numpy matplotlib seaborn scipy statsmodels opencv-python scikit-learn plotly pymorphy3 wordcloud
```

<details>
<summary><b>📦 Подробнее о библиотеках</b> (нажмите, чтобы развернуть)</summary>

| Библиотека | Назначение |
|------------|------------|
| `pandas` | Загрузка и обработка данных |
| `numpy` | Численные вычисления |
| `matplotlib` | Построение графиков |
| `seaborn` | Статистическая визуализация |
| `scikit-learn` | Машинное обучение, TF-IDF |
| `statsmodels` | Анализ временных рядов |
| `opencv-python` | Работа с изображениями |
| `PIL` | Обработка изображений |
| `pymorphy3` | Лемматизация русского текста |
| `wordcloud` | Облако слов |
| `plotly` | Интерактивные графики |

</details>

---

### 3️⃣ Скачивание данных

| Тип данных | Источник | 📥 Инструкция |
|------------|----------|---------------|
| **📊 Табличные** | [Kaggle](https://www.kaggle.com/datasets/mlg-ulb/creditcardfraud) | Скачать `creditcard.csv` → `data/table/` |
| **📈 Временные ряды** | [Yahoo Finance](https://finance.yahoo.com/quote/NVDA/history) | Период 2016-2026 → Download → `data/timeseries/` |
| **🖼️ Изображения** | Собственный датасет | Разложить по папкам: `train/helmet/`, `train/no_helmet/`, `test/helmet/`, `test/no_helmet/` |
| **📝 Текстовые** | Создается автоматически | Код `chapter4_text.py` сам создаст файл `data/text/appeals.csv` |

---

### 4️⃣ Запуск анализа

```bash
# Перейдите в папку с кодом
cd D:\2 семестр\курсач\code

# Запустите анализ по главам
python chapter1_table.py
python chapter2_timeseries.py
python chapter3_images.py
python chapter4_text.py
```

---

### 5️⃣ Результаты

Все рисунки и графики сохраняются в папку `results/`

---

## 📊 Результаты анализа

<details>
<summary><b>📊 Глава 1: Табличные данные</b> (нажмите, чтобы развернуть)</summary>

| Показатель | Значение |
|------------|----------|
| Размер датасета | 284 807 транзакций |
| Мошеннических транзакций | 492 (0.173%) |
| Ключевые признаки | V17 (-0.326), V14 (-0.303), V11 (+0.155) |
| Выбросы в Amount | 31 685 (11.17%) |
| Дубликаты | 1081 (удалены) |
| Пропуски | отсутствуют |

</details>

<details>
<summary><b>📈 Глава 2: Временные ряды</b> (нажмите, чтобы развернуть)</summary>

| Показатель | Значение |
|------------|----------|
| Период | 2016–2025 (2514 торговых дней) |
| Рост цены | с 0.62$ до 207$ (более чем в 330 раз) |
| SNR (отношение сигнал/шум) | 23.26 дБ (отлично) |
| Корреляция цена-объем | -0.36 |
| Максимальная волатильность | 4.5% (2023, 2025) |

</details>

<details>
<summary><b>🖼️ Глава 3: Изображения</b> (нажмите, чтобы развернуть)</summary>

| Показатель | Значение |
|------------|----------|
| Размер датасета | 5 000 изображений |
| Баланс классов | 91.6% (helmet) / 8.4% (no_helmet) |
| Разрешение | 415×416 пикселей |
| Выявленные проблемы | Дисбаланс классов, недостаточное разрешение |

</details>

<details>
<summary><b>📝 Глава 4: Текстовые данные</b> (нажмите, чтобы развернуть)</summary>

| Показатель | Значение |
|------------|----------|
| Размер датасета | 40 обращений, 7 категорий |
| Наиболее частые слова | улица (36), просить (35), дом (20) |
| Размер словаря TF-IDF | 200 слов |
| Максимальная похожесть поиска | 0.63 |

</details>

---

## 📚 Источники данных

| Датасет | Ссылка |
|---------|--------|
| **Credit Card Fraud Detection** | [🔗 Kaggle](https://www.kaggle.com/datasets/mlg-ulb/creditcardfraud) |
| **NVIDIA Daily Stock Prices** | [🔗 Yahoo Finance](https://finance.yahoo.com/quote/NVDA/history) |
| **Hard Hat Workers Dataset** | [🔗 Roboflow](https://universe.roboflow.com/) |
| **Обращения граждан** | Синтетический датасет (создан в коде) |

---

## 📖 Использованные библиотеки

| Библиотека | Назначение |
|------------|------------|
| `pandas` | Загрузка и обработка данных |
| `numpy` | Численные вычисления |
| `matplotlib` | Построение графиков |
| `seaborn` | Статистическая визуализация |
| `scikit-learn` | Машинное обучение, TF-IDF |
| `statsmodels` | Анализ временных рядов |
| `opencv-python` | Работа с изображениями |
| `PIL` | Обработка изображений |
| `pymorphy3` | Лемматизация русского текста |
| `wordcloud` | Облако слов |
| `plotly` | Интерактивные графики |

---

## 🔗 Ссылки

| Ссылка | Описание |
|--------|----------|
| [GitHub Репозиторий](https://github.com/linkov7951/Course-Project) | Исходный код проекта |
| [Отчет по курсовой работе](./docs/Отчет_курсовая_работа.docx) | Полная документация |

---

## 👨‍💻 Об авторе

| Поле | Значение |
|------|----------|
| **Студент** | Линьков Андрей Дмитриевич |
| **Группа** | ЕТ-141 |
| **Университет** | Южно-Уральский государственный университет (НИУ) |
| **Центр** | ВиртУм |
| **GitHub** | [@linkov7951](https://github.com/linkov7951) |

---

## 📄 Лицензия

<div align="center">

Данный проект выполнен в учебных целях.  
Использованные датасеты распространяются под лицензиями **CC BY-NC 4.0** и **Open Database License**.

</div>
```
